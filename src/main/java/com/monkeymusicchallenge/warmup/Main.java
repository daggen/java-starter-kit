...
public class Main {
	...
	private static final String APIKEY = "lJ24ZmReZ1wdg6bzLbqo4gaMcIE=";
	private static final String TEAM_NAME = "GummiBears";
	
	// You control your monkey by sending POST requests to the Monkey Music
	// server
	private static final String SERVER_URL = "http://warmup.monkeymusicchallenge.com";

	public static void main(String[] args) {
		if (args.length == 0) {
			args = new String[] {TEAM_NAME, APIKEY};
		}
		// Don't forget to provide the right command line arguments
		if (args.length < 2) {
			System.out
					.println("Usage: java -jar target/warmup.jar <your-team-name> <your-api-key\n");
			if (args.length < 1) {
		...
		// When we POST a command to the server, it always replies with the
		// current game state
		JSONObject currentGameState = postToServer(teamUrl, "command",
				"new game", "apiKey", apiKey);
		...
		while (currentGameState.getInt("turns") > 0) {
			...
			currentGameState = postToServer(teamUrl, "command", "move",
					"direction", nextMoveDirection, "apiKey", apiKey);
		...
		System.out.println("\nGame over!\n");
		// Go to http://warmup.monkeymusicchallenge.com/team/<your-team-name>
		// for more details
		System.out.println("  " + currentGameState.getString("message"));
		System.exit(0);
	}

	// Every time we POST a command to the server, we get the current game state
	// back
	private static JSONObject postToServer(final String teamUrl,
			final String... args) {
		...
		try {
			// In this starter kit, we use the Unirest library to send POST
			// requests
			final HttpResponse<JsonNode> response = Unirest.post(teamUrl)
					.header("Content-Type", "application/json")
					.body(new JSONObject(messageToPost).toString()).asJson();

			if (response.getCode() == 200) {
				return response.getBody().getObject();
			} else {
				throw new RuntimeException("Server replied with status code "
						+ response.getCode() + ", "
			...
			e.printStackTrace();
			System.out
					.println("Error! We seem to have  trouble talking to the server...\n");
